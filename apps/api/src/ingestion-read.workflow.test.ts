...
describe("Phase 2.9.1 ingestion read-back workflow", () => {
  ...
  it("performs ingestion first and reads the committed records back through unified query", async () => {
    ...
    expect(output).toEqual({ ingestion: result, readBack: { source: result.source, entities: [result.entities[0]], assertions: [result.assertions[0]], relationships: [result.relationships[0]], evidence: [result.evidence[0]], provenance: [result.provenance[0]] } });
  });
  ...
  it("fails the whole workflow when any child read-back is missing", async () => {
    ...
    const workflow = createIngestionReadWorkflow({ ingestion, query: query as never });
    await expect(workflow.execute({ source: { id: "source-1", kind: "repository" } } as never)).rejects.toMatchObject({ name: "ApplicationError", code: "STORAGE_ERROR", message: "Committed entity entity-1 was not found during workflow read-back" });
  });
